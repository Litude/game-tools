...
BMP* BMP_ReadFile( const char* filename )
{
	...
	/* If header contains extra data, skip it and update other stuff to compensate */
	if (bmp->Header.HeaderSize > 40)
	{
		UINT skip = bmp->Header.HeaderSize - 40;
		fseek(f, skip, SEEK_CUR);
		bmp->Header.HeaderSize -= skip;
		bmp->Header.FileSize -= skip;
		bmp->Header.DataOffset -= skip;		
	}
	...
	if ( bmp->Header.BitsPerPixel == 8 )
	{
		...
		if (bmp->Header.ColorsUsed) {
			memset(bmp->Palette, 0, BMP_PALETTE_SIZE);
			error = fread(bmp->Palette, sizeof(UCHAR), bmp->Header.ColorsUsed * 4, f) != bmp->Header.ColorsUsed * 4;
		} else {
			error = fread(bmp->Palette, sizeof(UCHAR), BMP_PALETTE_SIZE, f) != BMP_PALETTE_SIZE;
		}
