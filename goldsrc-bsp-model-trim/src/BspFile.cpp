...
bool BspFile::writeLump(LumpType type, std::ofstream &filestream) {
	constexpr char padding[] = { 0, 0, 0, 0 };
...
void BspFile::replaceLump(LumpType type, BspLump* newLump) {
	//std::memcpy(&this->lumps[type], newLump, sizeof(BspLump));
	//std::cout << "Copied";
	this->lumps[type] = std::move(*newLump);
	//std::cout << newLump->data.get();
	//this->lumps[type].length = newLump->length;
	//this->lumps[type].offset = newLump->offset;
	//this->lumps[type].data = std::move(newLump->data);
}
