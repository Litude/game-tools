...
BMP* BMP_ReadFile( const char* filename )
{
	...
	// Skip extra header information
	if (bmp->Header.HeaderSize > 40) {
		fseek( f, bmp->Header.HeaderSize - 40, SEEK_CUR );
	}
...
/**************************************************************
	Create rotate copy of original BMP only supports multiples
	of 90 degrees
**************************************************************/
BMP* BMP_Rotated(BMP* bmp, INT degrees)
{
	if (degrees % 90 != 0) {
		BMP_LAST_ERROR_CODE = BMP_INVALID_ARGUMENT;
		return false;
	...
	INT rotationCount = (degrees / 90) % 4;
	// Flip negative rotation to positive
	if (rotationCount < 0) {
	...
	for (INT i = 0; i < rotationCount; ++i) {
		BMP* newBmp = RotateRight(rotated);
		if (!newBmp) {
			if (rotated != bmp) {
				BMP_Free(rotated);
			}
...
/**************************************************************
	Flip BMP verically
**************************************************************/
BMP* BMP_FlippedVertically(BMP* bmp)
{
	UINT bmpHeight = BMP_GetHeight(bmp);
	UINT bmpWidth = BMP_GetWidth(bmp);
	BMP* bmpFlipped = BMP_Create(bmpWidth, bmpHeight, 32);
	if (!bmpFlipped)
	{
		BMP_LAST_ERROR_CODE = BMP_OUT_OF_MEMORY;
		return NULL;
	...
	for (UINT y = 0; y < bmpHeight; ++y) {
		for (UINT x = 0; x < bmpWidth; ++x) {
			...
			BMP_SetPixelRGB(bmpFlipped, x, bmpHeight - y - 1, red, green, blue);
		}
	}
...
/**************************************************************
	Flip BMP horizontally
**************************************************************/
BMP* BMP_FlippedHorizontally(BMP* bmp)
{
	UINT bmpHeight = BMP_GetHeight(bmp);
	UINT bmpWidth = BMP_GetWidth(bmp);
	BMP* bmpFlipped = BMP_Create(bmpWidth, bmpHeight, 32);
	...
	for (UINT y = 0; y < bmpHeight; ++y) {
		for (UINT x = 0; x < bmpWidth; ++x) {
			UCHAR red = 0, green = 0, blue = 0;
			BMP_GetPixelRGB(bmp, x, y, &red, &green, &blue);
			BMP_SetPixelRGB(bmpFlipped, bmpWidth - x - 1, y, red, green, blue);
		}
	}
...
/**************************************************************
	Returns a copy of the BMP scaled to newWidth x newHeight
**************************************************************/
BMP* BMP_Scaled(BMP* bmp, UINT newWidth, UINT newHeight)
{
	BMP* bmpScaled = BMP_Create(newWidth, newHeight, 32);
	if (!bmpScaled)
	{
		BMP_LAST_ERROR_CODE = BMP_OUT_OF_MEMORY;
		return NULL;
	...
	UINT originalHeight = BMP_GetHeight(bmp);
	UINT originalWidth = BMP_GetWidth(bmp);
	...
	for (UINT y = 0; y < newHeight; ++y) {
		for (UINT x = 0; x < newWidth; ++x) {
			...
			UCHAR red = 0, green = 0, blue = 0;
			BMP_GetPixelRGB(bmp, srcX, srcY, &red, &green, &blue);
			BMP_SetPixelRGB(bmpScaled, x, y, red, green, blue);
		}
	}
...
BMP* RotateRight(BMP* bmp)
{
	UINT originalHeight = BMP_GetHeight(bmp);
	UINT originalWidth = BMP_GetWidth(bmp);
	BMP* bmpRotated = BMP_Create(originalHeight, originalWidth, 32);
	if (!bmpRotated)
	{
		BMP_LAST_ERROR_CODE = BMP_OUT_OF_MEMORY;
		return NULL;
	...
	for (UINT y = 0; y < originalHeight; ++y) {
		for (UINT x = 0; x < originalWidth; ++x) {
			UCHAR red = 0, green = 0, blue = 0;
			BMP_GetPixelRGB(bmp, x, y, &red, &green, &blue);
			BMP_SetPixelRGB(bmpRotated, originalHeight - y - 1, x, red, green, blue);
		}
	}
