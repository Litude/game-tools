...
int main (int argc, char *argv[])
{
	...
	char* newLights = malloc(input->lump[LUMP_LIGHTING].length);
	...
	for (uint32_t i = 0; i < face_count; ++i) {
		...
		if (faces[i].nLightmapOffset != UINT32_MAX) {
		...
		memcpy(newLights + faces[i].nLightmapOffset, input->lump[LUMP_LIGHTING].data + faces[i].nLightmapOffset, original_lsize.x * original_lsize.y * 3);
		//curOffset += original_lsize.x * original_lsize.y * 3;
		}
	}
